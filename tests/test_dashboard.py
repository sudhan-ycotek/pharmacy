...
def test_dashboard_profit_made_admin_only(app, client):
    with app.app_context():
        ...
        create_sale(owner_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 4}])
...
def test_dashboard_shows_expiry_list_widget(admin_client, app):
    with app.app_context():
        ...
        far_batch = make_batch(medicine_id, "Tablet", 5, expiry_date="2031-01-01",
                                cost_price_per_base_unit=1.0, mrp_per_base_unit=2.0)
        expired_batch = make_batch(medicine_id, "Tablet", 5, expiry_date="2032-01-01",
                                    cost_price_per_base_unit=1.0, mrp_per_base_unit=2.0)
        db = get_db()
        ...
        db.execute("UPDATE medicine_batches SET expiry_date = date(?, '+60 days') WHERE id = ?",
                   (today, far_batch))
        db.execute("UPDATE medicine_batches SET expiry_date = date(?, '-1 days') WHERE id = ?",
                   (today, expired_batch))
        db.commit()

    body = admin_client.get("/").get_data(as_text=True)
    assert "Expiry List" in body
    # soon_batch's remaining qty (10) should render; far/expired batches must not add extra rows
    import re
    assert re.search(r"Expiry List.*?<table", body, re.DOTALL)
...
def test_dashboard_shows_recent_transactions_and_view_all_link(admin_client, app):
    with app.app_context():
        ...
        batch_id = make_batch(medicine_id, "Tablet", 1000, cost_price_per_base_unit=1.0, mrp_per_base_unit=2.5)
        user_id = create_user("cashier1", "pw", "staff")
        for _ in range(6):
...
def test_dashboard_recent_transactions_filtered_by_role(app, client):
    with app.app_context():
        ...
        create_sale(owner_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 1}])
