...
def _setup_medicine(app, stock_boxes=5, max_discount_percent=0, name="Cetamol"):
    with app.app_context():
        ...
        batch_id = make_batch(medicine_id, "Box", stock_boxes,
                               cost_price_per_base_unit=1.0, mrp_per_base_unit=2.5)
        return medicine_id, batch_id
...
def test_create_sale_decrements_stock_and_computes_total(app):
    ...
    with app.app_context():
        ...
        result = create_sale(user_id, [
            ...
            {"medicine_id": medicine_id, "unit_name": "File", "batch_id": batch_id, "quantity": 1},
        ])
...
def test_create_sale_rejects_insufficient_stock(app):
    medicine_id, batch_id = _setup_medicine(app, stock_boxes=1)
    with app.app_context():
        from auth import create_user
        user_id = create_user("staff1", "pw", "staff")
        ...
        with pytest.raises(ValueError):
            create_sale(user_id, [
                {"medicine_id": medicine_id, "unit_name": "File", "batch_id": batch_id, "quantity": 100},
            ])


def test_create_sale_rejects_box_unit_as_unsellable(app):
    medicine_id, batch_id = _setup_medicine(app, stock_boxes=5)
    with app.app_context():
        from auth import create_user
        user_id = create_user("staff1", "pw", "staff")
        ...
        with pytest.raises(ValueError):
            create_sale(user_id, [
                {"medicine_id": medicine_id, "unit_name": "Box", "batch_id": batch_id, "quantity": 1},
            ])
...
def test_create_sale_rejects_unknown_unit(app):
    ...
    with app.app_context():
        ...
        with pytest.raises(ValueError):
            create_sale(user_id, [
                {"medicine_id": medicine_id, "unit_name": "Pallet", "batch_id": batch_id, "quantity": 1},
            ])
...
def test_void_sale_twice_raises(app):
    ...
    with app.app_context():
        ...
        user_id = create_user("staff1", "pw", "staff")
        result = create_sale(user_id, [
            {"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 1},
        ])
        void_sale(result["sale_id"])
        with pytest.raises(ValueError):
            void_sale(result["sale_id"])
...
def test_void_sale_restores_exact_batch_used(app):
    """Voiding must restore the specific batch sold from, not just the medicine's aggregate."""
    with app.app_context():
        medicine_id = make_box_file_medicine(name="Cetamol", low_stock_threshold=50)
        batch_a = make_batch(medicine_id, "Box", 5, expiry_date="2030-01-01",
                              cost_price_per_base_unit=1.0, mrp_per_base_unit=2.5)
        batch_b = make_batch(medicine_id, "Box", 5, expiry_date="2031-01-01",
                              cost_price_per_base_unit=1.5, mrp_per_base_unit=3.0)
        from auth import create_user
        user_id = create_user("staff1", "pw", "staff")
        result = create_sale(user_id, [
            {"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_a, "quantity": 4},
        ])
        void_sale(result["sale_id"])
        assert get_batch(batch_a)["quantity_remaining"] == 5 * 240
        assert get_batch(batch_b)["quantity_remaining"] == 5 * 240
...
def test_today_sales_total_excludes_voided(app):
    ...
    with app.app_context():
        from auth import create_user
        user_id = create_user("staff1", "pw", "staff")
        r1 = create_sale(user_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 4}])
        create_sale(user_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 2}])
...
def test_sales_search_route_returns_matching_medicines(admin_client, app):
    ...
    assert "price" not in data[0]["units"][0]
...
def test_finalize_sale_route_creates_sale_and_returns_redirect(admin_client, app):
    ...
    response = admin_client.post(
        "/sales",
        json={"items": [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 2}]},
    )
    assert response.status_code == 200
    data = response.get_json()
...
def test_void_route_requires_admin(staff_client, app):
    ...
    with app.app_context():
        ...
        result = create_sale(user_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 1}])
...
def test_create_sale_rejects_duplicate_items_exceeding_stock(app):
    ...
    with app.app_context():
        from auth import create_user
        user_id = create_user("staff1", "pw", "staff")
        # Two line items for the same batch, together exceeding that batch's stock (1 box = 240 base units).
        # Each line item is 7 files = 140 base units, so 2 lines = 280 exceeds 240 stock.
        with pytest.raises(ValueError):
            create_sale(user_id, [
                {"medicine_id": medicine_id, "unit_name": "File", "batch_id": batch_id, "quantity": 7},
                {"medicine_id": medicine_id, "unit_name": "File", "batch_id": batch_id, "quantity": 7},
            ])
        medicine = get_medicine(medicine_id)
...
def test_create_sale_sells_against_two_batches_of_same_medicine(app):
    with app.app_context():
        medicine_id = make_box_file_medicine(name="Cetamol", low_stock_threshold=50)
        batch_a = make_batch(medicine_id, "File", 1, expiry_date="2030-01-01",
                              cost_price_per_base_unit=1.0, mrp_per_base_unit=2.0)
        batch_b = make_batch(medicine_id, "File", 10, expiry_date="2031-01-01",
                              cost_price_per_base_unit=1.2, mrp_per_base_unit=2.5)
        from auth import create_user
        user_id = create_user("staff1", "pw", "staff")
        # batch_a only has 1 file (20 base units); requesting 1 file from it and 5 files from batch_b succeeds.
        create_sale(user_id, [
            {"medicine_id": medicine_id, "unit_name": "File", "batch_id": batch_a, "quantity": 1},
            {"medicine_id": medicine_id, "unit_name": "File", "batch_id": batch_b, "quantity": 5},
        ])
        assert get_batch(batch_a)["quantity_remaining"] == 0
        assert get_batch(batch_b)["quantity_remaining"] == 5 * 20


def test_create_sale_batch_specific_oversell_rejected_even_with_sibling_stock(app):
    with app.app_context():
        medicine_id = make_box_file_medicine(name="Cetamol", low_stock_threshold=50)
        short_batch = make_batch(medicine_id, "File", 1, expiry_date="2030-01-01",
                                  cost_price_per_base_unit=1.0, mrp_per_base_unit=2.0)
        make_batch(medicine_id, "File", 10, expiry_date="2031-01-01",
                   cost_price_per_base_unit=1.2, mrp_per_base_unit=2.5)
        from auth import create_user
        user_id = create_user("staff1", "pw", "staff")
        # short_batch only has 1 file's worth — requesting 2 from it must fail even though
        # the medicine's other batch has plenty.
        with pytest.raises(ValueError):
            create_sale(user_id, [
                {"medicine_id": medicine_id, "unit_name": "File", "batch_id": short_batch, "quantity": 2},
            ])


def test_create_sale_item_mode_discount_capped_per_medicine(app):
    medicine_id, batch_id = _setup_medicine(app, max_discount_percent=10)
    with app.app_context():
        from auth import create_user
        user_id = create_user("staff1", "pw", "staff")
        result = create_sale(user_id, [
            {"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id,
...
def test_create_sale_bill_mode_discount_capped_at_minimum_across_items(app):
    with app.app_context():
        ...
        low_cap_batch = make_batch(low_cap_id, "Tablet", 100, cost_price_per_base_unit=1.0, mrp_per_base_unit=2.0)
        high_cap_id = make_box_file_medicine(name="Napa", low_stock_threshold=50, max_discount_percent=20)
        high_cap_batch = make_batch(high_cap_id, "Tablet", 100, cost_price_per_base_unit=1.0, mrp_per_base_unit=3.0)
        from auth import create_user
        user_id = create_user("staff1", "pw", "staff")
        items = [
            {"medicine_id": low_cap_id, "unit_name": "Tablet", "batch_id": low_cap_batch, "quantity": 2},
            {"medicine_id": high_cap_id, "unit_name": "Tablet", "batch_id": high_cap_batch, "quantity": 2},
        ]
        with pytest.raises(ValueError):
...
def test_create_sale_rejects_fractional_quantity(app):
    ...
    with app.app_context():
        ...
        with pytest.raises(ValueError):
            create_sale(user_id, [
                {"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 2.5},
            ])
        medicine = get_medicine(medicine_id)
...
def test_create_sale_rejects_bool_quantity(app):
    ...
    with app.app_context():
        ...
        with pytest.raises(ValueError):
            create_sale(user_id, [
                {"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": True},
            ])
...
def test_finalize_route_rejects_non_numeric_quantity(admin_client, app):
    ...
    response = admin_client.post(
        "/sales",
        json={"items": [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": "not_a_number"}]},
    )
    assert response.status_code == 400
    data = response.get_json()
    assert "error" in data
...
def test_finalize_route_rejects_fractional_quantity(admin_client, app):
    ...
    response = admin_client.post(
        "/sales",
        json={"items": [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 2.5}]},
    )
    assert response.status_code == 400
    data = response.get_json()
    assert "error" in data
...
def test_list_sales_route_admin_sees_all_staff_sees_own(app, client):
    ...
    with app.app_context():
        ...
        create_sale(admin_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 1}])
        create_sale(staff_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 2}])
...
def test_list_sales_business_logic_filters_by_user(app):
    ...
    with app.app_context():
        ...
        create_sale(user_a, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 1}])
        create_sale(user_b, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 1}])
...
def test_get_sale_includes_seller_username(app):
    ...
    with app.app_context():
        ...
        result = create_sale(user_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 2}])
        sale = get_sale(result["sale_id"])
...
def test_list_sales_includes_seller_username(app):
    ...
    with app.app_context():
        from auth import create_user
        user_id = create_user("cashier1", "pw", "staff")
        create_sale(user_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 1}])
        sales = list_sales()
...
def test_receipt_hides_cost_and_profit_from_staff_but_shows_admin(app, client):
    ...
    with app.app_context():
        ...
        result = create_sale(owner_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 2}])
...
def test_sales_list_hides_profit_from_staff_but_shows_admin(app, client):
    ...
    with app.app_context():
        ...
        create_user("boss", "bosspass", "admin")
        owner_id = create_user("owner", "ownerpass", "staff")
        create_sale(owner_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 2}])
...
def test_receipt_route_forbids_other_staff_but_allows_owner_and_admin(app, client):
    ...
    with app.app_context():
        ...
        result = create_sale(owner_id, [{"medicine_id": medicine_id, "unit_name": "Tablet", "batch_id": batch_id, "quantity": 1}])
    sale_id = result["sale_id"]
...
def test_medicine_batches_route_returns_available_batches_sorted_by_expiry(admin_client, app):
    with app.app_context():
        medicine_id = make_box_file_medicine(name="Cetamol", low_stock_threshold=50)
        make_batch(medicine_id, "Tablet", 10, expiry_date="2031-01-01",
                   cost_price_per_base_unit=1.0, mrp_per_base_unit=2.0)
    ...
    data = response.get_json()
    assert len(data) == 2
    assert data[0]["expiry_date"] == "2030-01-01"
    assert data[1]["expiry_date"] == "2031-01-01"
    for batch in data:
        assert "cost_price_per_base_unit" not in batch


def test_medicine_batches_route_excludes_depleted_batches(admin_client, app):
    with app.app_context():
        from inventory import remove_stock
        medicine_id = make_box_file_medicine(name="Cetamol", low_stock_threshold=50)
        batch_id = make_batch(medicine_id, "Tablet", 1, expiry_date="2030-01-01",
                               cost_price_per_base_unit=1.0, mrp_per_base_unit=2.0)
